...
- 先规划段落押韵归属和逐行音节网格，再检查韵脚、句长与重复 Hook 的稳定性。
- 为安静、克制类歌曲设置情绪与音域上限，清除会误导升 Key、飙高音或全编制高潮的冲突指令。
...
- 中文语序是否自然，是否为了押韵强行倒装。
- 每段是否预先设定韵脚家族，尤其结句是否回到该段的归韵。
- 对应歌词行是否遵循稳定的演唱音节网格，而不是只落在宽泛的字数区间内。
- Verse 2 是否真正增加事实、后果或新的理解。
- Hook 是否口语化、好记、能代表全歌。
- 不规则短句、停顿和省略是否有角色与演唱功能。
- 结尾是否回收或改变前面出现过的细节。
- 歌词是否保留了用户有辨识度的表达，而不是被统一润色成“标准 AI 文风”。
- 安静歌曲的副歌是否只加深语义而不自动升高、变响；Lyrics 与 Style 中是否还残留 `Build`、`Belting`、`Full Band Lift` 或 `Key Lift` 等冲突提示。
